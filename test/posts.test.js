...
describe('Posts API', () => {
    const cleanupTestPost = (title) => __awaiter(void 0, void 0, void 0, function* () {
        const post = yield (0, knex_1.default)('posts').where({ title }).first();
        if (post) {
            yield (0, knex_1.default)('posts').where({ id: post.id }).delete();
        }
    });
    const cleanupTestUser = (email) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        if (user) {
            yield (0, knex_1.default)('posts').where({ user_id: user.id }).delete();
    ...
    afterAll(() => __awaiter(void 0, void 0, void 0, function* () {
        yield cleanupTestUser(`posttest_${testUserId}@example.com`);
    }));
    describe('POST /posts', () => {
        it('should create a post', () => __awaiter(void 0, void 0, void 0, function* () {
            const testTitle = `Test Post ${Date.now()}`;
            yield cleanupTestPost(testTitle);
    ...
    describe('GET /posts?userId={userId}', () => {
        it('should return posts for a user', () => __awaiter(void 0, void 0, void 0, function* () {
            const testTitle = `Test Post ${Date.now()}`;
            yield cleanupTestPost(testTitle);
            yield (0, knex_1.default)('posts').insert({
                user_id: testUserId,
                title: testTitle,
                body: 'Content',
                created_at: new Date().toISOString()
            });
            const res = yield (0, supertest_1.default)(server_1.default).get(`/posts?userId=${testUserId}`);
            expect(res.status).toBe(200);
            expect(Array.isArray(res.body)).toBe(true);
            expect(res.body[0]).toHaveProperty('title', testTitle);
            yield cleanupTestPost(testTitle);
        ...
        it('should return 400 if userId is missing', () => __awaiter(void 0, void 0, void 0, function* () {
            const res = yield (0, supertest_1.default)(server_1.default).get('/posts');
            expect(res.status).toBe(400);
            expect(res.body).toHaveProperty('message', 'userId query parameter is required');
        }));
    });
    describe('DELETE /posts/:id', () => {
        it('should delete a post', () => __awaiter(void 0, void 0, void 0, function* () {
            const testTitle = `Test Post ${Date.now()}`;
            yield cleanupTestPost(testTitle);
            const [postId] = yield (0, knex_1.default)('posts').insert({
                user_id: testUserId,
                title: testTitle,
                body: 'Content',
                created_at: new Date().toISOString()
            });
            const res = yield (0, supertest_1.default)(server_1.default).delete(`/posts/${postId}`);
            expect(res.status).toBe(204);
        }));
        it('should return 404 for non-existent post', () => __awaiter(void 0, void 0, void 0, function* () {
            const nonExistentId = 9999;
            yield (0, knex_1.default)('posts').where({ id: nonExistentId }).delete();
            const res = yield (0, supertest_1.default)(server_1.default).delete(`/posts/${nonExistentId}`);
            expect(res.status).toBe(404);
            expect(res.body).toHaveProperty('message', 'Post not found');
        }));
    });
